...
with cte as (
select Cuisine, Restaurant_id, count(1) as n_orders, 
dense_rank() over(partition by Cuisine order by count(1) desc) as rnk from orders group by Cuisine, Restaurant_id )
select Cuisine, Restaurant_id from cte where rnk = 1;

--Analyzing daily new customer acquisition from first order date
with cte as 
(select Customer_code, cast(min(Placed_at) as date) as  first_order_date from orders group by Customer_code )
select first_order_date , count(*) as n_new_customers from cte group by first_order_date

-- Count of all the users who were acquired in Jan 2025 and only placed one order in Jan and then never placed any other order
select  Customer_code, count(*) as n_orders from orders where month(Placed_at) = 1 and year(Placed_at) = 2025
and Customer_code not in (select distinct Customer_code from orders where not(month(Placed_at) = 1 and year(Placed_at) = 2025))
...
Max(placed_at) as latest_order_date from orders group by Customer_code)
select cte.*, orders.Promo_code_Name as first_order_promo from cte inner join orders 
...
-- Growth team is planning to create a trigger that will target customers after their every 3rd order with a personalised communication and they have asked to create a query for this
with cte as
(select *, row_number() over(partition by Customer_code order by Placed_at) as order_number from orders)
select * from cte where order_number%3 =0 and cast(Placed_at as date)= cast(GETDATE() as date);

--List customers who placed more than 1 order and all their orders on promo only 
select Customer_code, count(*) as n_orders, count(Promo_code_Name) as promo_orders from orders group by Customer_code
having count(*) >1 and count(*) = count(Promo_code_Name) ;

--What percent of customers were organically acquired in Jan 2025, placed their 1st order without promo code 
with cte as (
select *, row_number() over(partition by Customer_code order by Placed_at) as rn from orders where month(Placed_at) = 1)
select count(case when rn = 1 and Promo_code_Name is null  then Customer_code end)*100.0/ count(distinct Customer_code) as acquired_percent from cte 